...
/mob/living/simple_animal/parrot
	...
	turns_per_move = 5
	butcher_results = list(/obj/item/weapon/reagent_containers/snacks/cracker/ = 1)
...
/mob/living/simple_animal/parrot/attackby(obj/item/O, mob/living/user, params)
	if(!stat && !client && !istype(O, /obj/item/stack/medical) && !istype(O,/obj/item/weapon/reagent_containers/snacks/cracker))
		if(O.force)
			if(parrot_state == PARROT_PERCH)
				parrot_sleep_dur = parrot_sleep_max //Reset it's sleep timer if it was perched
			...
			icon_state = "parrot_fly"
			drop_held_item(0)
	else if(istype(O,/obj/item/weapon/reagent_containers/snacks/cracker)) //Poly wants a cracker.
		qdel(O)
		user.drop_item()
		if(health < maxHealth)
			adjustBruteLoss(-10)
...
//parrots will eat crackers instead of dropping them
	if(istype(held_item,/obj/item/weapon/reagent_containers/snacks/cracker) && (drop_gently))
		qdel(held_item)
		held_item = null
		if(health < maxHealth)
			adjustBruteLoss(-10)
