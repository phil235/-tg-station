...
/obj/machinery/microwave/proc/cook()
	...
	if (prob(max(dirty*5-efficiency,0))) //a clean unupgraded microwave has no risk of failure
		muck_start()
		if (!microwaving(4))
			muck_finish()
			return
...
/obj/machinery/microwave/proc/microwaving(var/seconds as num)
	for (var/i=1 to seconds)
		if (stat & (NOPOWER|BROKEN))
			return 0
		use_power(500)
		sleep(max(12-2*efficiency,2)) // standard power means sleep(10). The higher the power and the better the efficiency, the faster the cooking
	return 1
